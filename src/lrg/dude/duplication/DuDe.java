...
public class DuDe {
    ...
    public static void main(String[] args) throws IOException {
        ...
        for(int index = 0; index < results.length; index++) {
        	...
        	if(results[index].isSelfDuplication()) continue;
	...
	private static List<ChronosImportJson> exportDuplication(String filename, List<Duplication> duplicationsForFile) {
		...
		if(filename.contains("CurrencyCloud."))  System.err.println(filename);
		for(Duplication crtDup : duplicationsForFile) {
			...
			if(filename.compareTo(crtDup.getReferenceCode().getEntityName()) == 0) {
				...
				if(filename.contains("CurrencyCloud.")) System.err.println("\t >>>" + crtDup.getDuplicateCode().getEntityName());
			}
			else duplicatedFiles.add(crtDup.getReferenceCode().getEntityName());
		}
